...
async def test_load_nonexistent_session_returns_default(manager: SessionManager):
    state = await manager.load_session("nonexistent_user", "nonexistent_char")
...
async def test_save_and_load_session(manager: SessionManager):
    ...
    await manager.save_session("test_user", "test_char", original)
    manager._cache.clear()

    loaded = await manager.load_session("test_user", "test_char")
...
async def test_repository_sanitizes_session_paths(session_repo: FileSessionRepository):
    ...
    await manager.save_session("../../../etc/user", "char:with|special*chars?", SessionState(session_id=1))
...
async def test_repository_rejects_empty_character_id(session_repo: FileSessionRepository):
    ...
    with pytest.raises(ValueError, match="char_id must be non-empty"):
        await manager.save_session("test_user", "", SessionState(session_id=1))
...
async def test_clear_history_preserves_metadata(manager: SessionManager):
    ...
    await manager.save_session("test_user", "test_char", state)

    await manager.clear_history("test_user", "test_char")

    reloaded = await manager.load_session("test_user", "test_char")
...
async def test_clear_session_deletes_file(session_repo: FileSessionRepository):
    manager = SessionManager(repo=session_repo)
    await manager.save_session("test_user", "test_char", SessionState(session_id=1))

    path = session_repo.root / "test_char_test_user.json"
    assert path.exists()

    await manager.clear_session("test_user", "test_char")
...
async def test_add_turn_with_history_limit(session_repo: FileSessionRepository):
    ...
    for index in range(5):
        await manager.add_turn("test_user", "test_char", f"User {index}", f"AI {index}")

    state = await manager.load_session("test_user", "test_char")
...
async def test_get_and_update_history(manager: SessionManager):
    await manager.save_session(
        "test_user",
        "test_char",
        SessionState(short_term_history=[{"role": "user", "content": "Old"}]),
    )
    ...
    await manager.update_history("test_user", "test_char", new_history)

    assert await manager.get_history("test_user", "test_char") == new_history


async def test_multiple_users_are_isolated(manager: SessionManager):
    await manager.save_session("user1", "char", SessionState(session_id=1, metadata={"user": 1}))
    await manager.save_session("user2", "char", SessionState(session_id=2, metadata={"user": 2}))

    loaded1 = await manager.load_session("user1", "char")
    loaded2 = await manager.load_session("user2", "char")
