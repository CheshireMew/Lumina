...
async def test_session_lifecycle(tmp_path: Path):
    ...
    session = await manager.load_session(user_id, char_id)
    assert isinstance(session, SessionState)
    assert session.short_term_history == []

    session.short_term_history.append({"role": "user", "content": "hello"})
    await manager.save_session(user_id, char_id, session)

    manager._cache.clear()
    session2 = await manager.load_session(user_id, char_id)
    assert session2.short_term_history == [{"role": "user", "content": "hello"}]

    await manager.clear_history(user_id, char_id)
    session3 = await manager.load_session(user_id, char_id)
    assert session3.short_term_history == []

    await manager.clear_session(user_id, char_id)
