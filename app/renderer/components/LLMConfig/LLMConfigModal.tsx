...
const LLMConfigModal: React.FC<LLMConfigModalProps> = ({
    ...
    const [overflowStrategy, setOverflowStrategy] = useState<"slide" | "reset">(
        "reset",
    ); // Default to Reset (Cache+)
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            // Use API_CONFIG.BASE_URL for checking if it matches local backend
            const cleanLocal = API_CONFIG.BASE_URL.replace(/\/$/, "");
            const isFree =
                url.includes("localhost:8010") ||
                url.includes("127.0.0.1:8010") ||
                url.includes(cleanLocal) ||
                !url;
    ...
    useEffect(() => {
        if (isOpen && providerType === "free") {
            ...
            fetch(`${API_CONFIG.BASE_URL}/models/list`)
                .then((res) => res.json())
                .then((data) => {
                    // Handle both array and {models: [...]} format
    ...
    const handleSave = () => {
        ...
        if (providerType === "free") {
            // Force Free settings
            finalBaseUrl = `${API_CONFIG.BASE_URL}/v1`; // Dynamic Local marker for Free Tier
                    ...
                    <div style={{ animation: "fadeIn 0.3s ease" }}>
                        {providerType === "free" ? (
                            ...
                            >
                                ...
                                >
                                    Privacy-focused proxy for OpenAI, Claude,
                                    and Llama. <br />
                                    <strong>
                            ...
                            >
                                ...
                                <button
                                    onClick={async () => {
                                        if (
                                            confirm(
                                                "Clear short-term memory (Context) for this session?",
                                            )
                                        ) {
                                            try {
                                                await fetch(
                                                    `${API_CONFIG.BASE_URL}/memory/context/clear`,
                                                    {
                                                        method: "POST",
                                                        headers: {
                                                            "Content-Type":
