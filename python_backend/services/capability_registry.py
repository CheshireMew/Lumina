...
class CapabilityRegistry:
    def __init__(self):
        ...
        self._providers_by_plugin: dict[str, list[CapabilityProvider]] = {}

    def register_plugin(self, plugin_id: str, capabilities: list[str], runtime_target: str, kind: str, enabled: bool):
        ...
        self._providers_by_plugin[plugin_id] = providers

    def unregister_plugin(self, plugin_id: str):
        providers = self._providers_by_plugin.pop(plugin_id, [])
        for provider in providers:
            current = self._providers_by_capability.get(provider.capability, [])
            self._providers_by_capability[provider.capability] = [
                item for item in current if item.plugin_id != plugin_id
            ]
    ...
    def set_enabled(self, plugin_id: str, enabled: bool):
        for provider in self._providers_by_plugin.get(plugin_id, []):
    ...
    def find_provider(
        ...
        for provider in candidates:
            ...
            if predicate and not predicate(provider):
                continue
            if selected_provider and provider.plugin_id == selected_provider:
