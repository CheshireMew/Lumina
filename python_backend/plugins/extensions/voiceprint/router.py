...
def get_voiceprint_manager() -> "VoiceprintManager":
    ...
    from .manager import VoiceprintManager as VM
...
async def list_profiles():
    ...
    # Use Async Accessor (IPC Compatible)
    if hasattr(mgr, 'get_all_profiles'):
        profiles = await mgr.get_all_profiles()
    else:
        # Fallback (Should not happen if updated)
        # But for safety if class reload failed
        mgr.reload_profiles()
        profiles = []
...
async def toggle_profile(name: str, enabled: bool):
    ...
    # Check if async (Remote Proxy) or sync (Local)
    import inspect
    if inspect.iscoroutinefunction(mgr.toggle_profile):
        result = await mgr.toggle_profile(name, enabled)
    else:
        result = mgr.toggle_profile(name, enabled)
