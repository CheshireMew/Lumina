...
class CapabilityModule(ABC):
    """
    Runtime-loaded capability module contract.
    ...
    def id(self) -> str:
        if not self._manifest:
            raise RuntimeError("Capability manifest was not injected.")
        return self._manifest.id

    @property
    def context(self):
        if self._context is None:
            raise RuntimeError(f"Capability module {self.__class__.__name__} has not been loaded.")
