...
export class TTSService implements ITTSProvider {
    ...
    async synthesize(text: string, voice?: string, engine?: string): Promise<import('./types').AudioResponse | null> {
        const requestVoice = voice || this.defaultVoice;
        const requestEngine = engine || this.defaultEngine;

        try {
            console.log(`[TTS] Request: text="${text.substring(0, 10)}...", voice=${requestVoice}, engine=${requestEngine}`);
            ...
            console.log(`[TTS] Response: type=${contentType}, status=${response.status}`);
    ...
    async listVoices(engine: string = 'edge-tts'): Promise<VoiceInfo[]> {
        try {
            const response = await fetch(`${this.baseUrl}/tts/voices?engine=${engine}`);
            if (!response.ok) {
