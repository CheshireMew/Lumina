...
export class AudioQueue {
    ...
    private playStream(item: QueueItem): Promise<void> {
        ...
        console.log(
            `[AudioQueue] playStream called for ${contentType}`,
            stream,
        );
        ...
        if (typeof stream.getReader !== "function") {
            console.error(
                "[AudioQueue] stream.getReader is not a function!",
                stream,
            );
            return Promise.reject(
                new Error("Stream does not have getReader method"),
            );
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.mediaSource) {
                try {
                    ...
                    console.warn(
                        "[AudioQueue] Failed to cleanup old MediaSource:",
                        e,
                    );
                }
            }
            ...
            mediaSource.addEventListener("sourceopen", async () => {
                console.log(
                    `[AudioQueue] MediaSource opened for ${contentType}`,
                );
                try {
                    ...
                    const processQueue = async () => {
                        while (queue.length > 0 && !sourceBuffer.updating) {
                            ...
                            try {
                                ...
                                console.error(
                                    "[AudioQueue] SourceBuffer append error:",
                                    e,
                                );
                    ...
                    audio
                        .play()
                        .catch((e) =>
                            console.warn("[AudioQueue] Auto-play failed:", e),
                        );
    ...
    clear() {
        ...
        if (this.currentAudio) {
            ...
            // ⚡ 关键修复：撤销 ObjectURL
            // ⚡ 关键修复：撤销 ObjectURL (Delay to avoid ERR_FILE_NOT_FOUND)
            if (this.currentAudio.src) {
                const src = this.currentAudio.src;
                // Detach source first
                this.currentAudio.removeAttribute("src");
