...
export class MemoryService {
        ...
        if (results.length > 0) {
          const formatted = results
            .map((res: any) => {
              ...
              const textContent =
                res.text || res.content || res.payload?.text || "(No content)";
   ...
   */
  public async consolidateHistory(
    messages: Message[],
    userName: string,
    charName: string
  ): Promise<void> {
    ...
    try {
      console.log(
        `[MemoryService] Consolidating ${messages.length} messages...`
      );
      const payload = {
        user_id: "user",
        character_id: this.currentCharacterId,
   ...
   */
  public async triggerDeepDreaming(
    userName: string,
    charName: string
  ): Promise<void> {
    if (!this.isConfigured) return;
    ...
    try {
      console.log(
        `[MemoryService] 🌙 Triggering Deep Dreaming for ${charName}...`
      );
      const payload = {
        user_id: "user",
        character_id: this.currentCharacterId,
